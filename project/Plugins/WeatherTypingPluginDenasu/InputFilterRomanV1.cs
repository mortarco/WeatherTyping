...
namespace com.denasu.WeatherTyping.plugin.input
{
    ...
    /// </summary>
    public class InputFilterRomanV1 : MarshalByRefObject, IInputFilter
    {
        /// <summary>
        /// Version
        /// </summary>
        ...
        internal void ProcessSokuonShortcut(List<ShortcutInfo> sokuonShortcuts)
        {
            foreach (var sokuonShortcut in sokuonShortcuts)
            {
                ...
                bool proc = false;
                foreach (var connection_from_xtu_or_ltu in sokuonShortcut.Second.GetConnect())
                {
                    if (sokuonShortcut.First.GetConnect().ContainsKey(connection_from_xtu_or_ltu.Key))
                    {
                        // To make it deterministic, use existing automaton (state_x or state_l)
                        var state_x_or_l = sokuonShortcut.First.GetConnect()[connection_from_xtu_or_ltu.Key].Automaton;

                        // Connect state_x to next state with x or l
                        state_x_or_l.SetConnect(connection_from_xtu_or_ltu.Key, new InputAutomaton.Connect()
                        {
                            Automaton = connection_from_xtu_or_ltu.Value.Automaton,
                            Character = connection_from_xtu_or_ltu.Value.Character,
                ...
                if (proc)
                {
                    // We processed this shortcut
                    continue;
                }

                // For sequencial of next first character (e.g. "っった" -> ttta)
                if (!sokuonShortcut.Hatuon)
                {
                    // Except for "っん"
                    foreach (var connection_from_xtu_or_ltu in sokuonShortcut.Last.GetConnect())
                    {
                        string tmp = "";
                        tmp += connection_from_xtu_or_ltu.Value.Character;
                        if (!_sokuonmap.Contains(tmp))
                        {
                            // Connect prev to next state with the first character of last
                            sokuonShortcut.First.SetConnect(connection_from_xtu_or_ltu.Key, new InputAutomaton.Connect()
                            {
                                Automaton = sokuonShortcut.Second,
                                Character = connection_from_xtu_or_ltu.Value.Character,
                                Flags = InputCustomRomanV1.Ope_NO_SMALL,
                            });
                        }
